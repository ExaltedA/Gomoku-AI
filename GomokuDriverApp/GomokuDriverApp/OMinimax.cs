...
namespace GomokuDriverApp
{
	...
	public class OMinimax
	{
		...
		public static double evaluateBoardFor(Board board, bool xTurn)
		{
			...
			double xScore = getScore(board, true, xTurn);
			double oScore = getScore(board, false, xTurn);
		...
		/*
		 * alpha : Best [X] Ai Move (Max)
		 * beta : Best [0] Ai Move (Min)
		...
		private static Object[] searchWinningMove(Board board)
		{
			...
			foreach (int[] move in allPossibleMoves)
			{
				...
				// If the X has a winning score in that temporary board, return the move.
